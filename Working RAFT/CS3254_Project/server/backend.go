...
/*
	...
	6.) We should label client messages with client addresses & command number so we know when the results for the client's request is ready
		This allows us to differiate between mutliple client requests and return the results in a total ordered fashion
		from the perspective of a single client

		client must track its own order # by incrementing it only.
*/
...
const HEARTBEAT_PERIOD = time.Duration(800) * time.Millisecond // Should be less than the min timeout time
const TIMEOUT_RANGE_START = 1000
const TIMEOUT_RANGE_END = 2000
...
func (self *Raft) SendCommand(req SendCommandArgs, res *SendCommandResponse) error {
	...
	fmt.Println("Printing State from SendCommand: Before")
	fmt.Println("THE COMMAND IS: ", req.Command)
	self.printState()
	...
	fmt.Println("SDFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF:" , self.committedIndex, logEntryIndex)
	...
	fmt.Println("Printing State from SendCommand: After")
	self.printState()
...
// This lets the follower's apply the commands and commit the log entries given from the leader
// All reads are basically ignored since they don't affect the final state of our data
// so we can just claim that we applied it
...
func (self *Raft) AppendEntries(req AppendEntriesArgs, res *AppendEntriesResponse) error {
	...
	// We have an inconsistency, we reject
	...
	fmt.Println("AppendEntries current follower's log: ", self.log)
	fmt.Println("AppendEntries current follower's committedIndex: ", self.committedIndex)
	fmt.Println("AppendEntries PrevLogIndex: ", req.PrevLogIndex)
	...
	if (self.committedIndex != req.LeaderCommit) {
	...
	// Leader's commit shouldn't be longer than our log at this point
	// ????? i <= req.LeaderCommit apparently break this
	...
	// #5 on Raft Research Paper (dont understand why min?)
	// Shouldn't leaderCommit always be less than the last Index
	if (req.LeaderCommit > self.committedIndex) {
		self.committedIndex = minInt(req.LeaderCommit, len(self.log)-1)
	}

	fmt.Println("AppendEntries after follower's log: ", self.log)
	fmt.Println("AppendEntries LeaderCommit: ", req.LeaderCommit)
	fmt.Println("AppendEntries after follower's committedIndex: ", self.committedIndex)
	fmt.Println("AppendEntries PrevLogIndex: ", req.PrevLogIndex)
	...
	for {
		...
		default:
			...
			for i, replicaAddr := range self.replicas {
				...
				fmt.Println("SENDING ENTIRES: ", req.Entries)
				//fmt.Println("heartbeat controller LOG: ", self.log)
				//fmt.Println("heartbeat controller nextIndex: ", self.nextIndex[i])
				RaftMutex.Unlock()
